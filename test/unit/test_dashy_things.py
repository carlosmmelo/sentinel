...
def test_valid_xsn_address():
    from xsnlib import is_valid_xsn_address

    main = valid_xsn_address()
    test = valid_xsn_address('testnet')

    assert is_valid_xsn_address(main) is True
    assert is_valid_xsn_address(main, 'mainnet') is True
    assert is_valid_xsn_address(main, 'testnet') is False

    assert is_valid_xsn_address(test) is False
    assert is_valid_xsn_address(test, 'mainnet') is False
    assert is_valid_xsn_address(test, 'testnet') is True


def test_invalid_xsn_address():
    from xsnlib import is_valid_xsn_address

    main = invalid_xsn_address()
    test = invalid_xsn_address('testnet')

    assert is_valid_xsn_address(main) is False
    assert is_valid_xsn_address(main, 'mainnet') is False
    assert is_valid_xsn_address(main, 'testnet') is False

    assert is_valid_xsn_address(test) is False
    assert is_valid_xsn_address(test, 'mainnet') is False
    assert is_valid_xsn_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from xsnlib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % xsnlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(xsnlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(xsnlib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(xsnlib.blocks_to_seconds(16616)) == 2612035
