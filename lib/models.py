...
class GovernanceObject(BaseModel):
    ...
    # sync xsnd gobject list with our local relational DB backend
    @classmethod
    def sync(self, xsnd):
        golist = xsnd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_xsnd(xsnd, item)
    ...
    @classmethod
    def import_gobject_from_xsnd(self, xsnd, rec):
        import decimal
        import xsnlib
        import inflection
        ...
        # shim/xsnd conversion
        object_hex = xsnlib.SHIM_deserialise_from_xsnd(object_hex)
        objects = xsnlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from xsnd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from xsnd, with every run
        ...
        # get/create, then sync payment amounts, etc. from xsnd - xsnd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(xsnd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from xsnd! %s" % e)
            govobj.vote_delete(xsnd)
            return (govobj, None)
    ...
    def vote_delete(self, xsnd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(xsnd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, xsnd, signal, outcome):
        import xsnlib
        ...
        output = xsnd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = xsnlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(xsnd, signal)

    def sync_network_vote(self, xsnd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = xsnd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = xsnD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 xsn addr, non-multisig
            if not xsnlib.is_valid_xsn_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid xsn address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (xsnlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/xsnDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import xsnlib
        obj_data = xsnlib.SHIM_serialise_for_xsnd(self.serialise())
    ...
    def prepare(self, xsnd):
        try:
            object_hash = xsnd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = xsnD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not xsnlib.is_valid_xsn_address(addr, config.network):
    ...
    def hash(self):
        import xsnlib
        return xsnlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = xsnD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, xsnd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - xsnd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, xsnd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - xsnd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, xsnd):
        now = int(time.time())
        return (self.created_at < (now - xsnd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, xsnd):
        if self.is_expired(xsnd):
            return False

        return True

    def is_deletable(self, xsnd):
        if self.is_expired(xsnd):
            return True
