...
def is_valid_xsn_address(address, network='mainnet'):
    ...
    xsn_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid xsn address
        return False

    if (address_version != xsn_version):
        return False
...
# shims 'til we can fix the xsnd side
def SHIM_serialise_for_xsnd(sentinel_hex):
    from models import xsnD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = xsnD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in xsnd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # xsnd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    xsnd_hex = serialise(obj)
    return xsnd_hex


# shims 'til we can fix the xsnd side
def SHIM_deserialise_from_xsnd(xsnd_hex):
    from models import xsnD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(xsnd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('xsn.conf')
