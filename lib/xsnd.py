...
class xsnDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some xsnd methods
    ...
    @classmethod
    def from_xsn_conf(self, xsn_dot_conf):
        from xsn_config import xsnConfig
        config_text = xsnConfig.slurp_config_file(xsn_dot_conf)
        creds = xsnConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = xsnlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = xsnlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = xsnlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_xsnd_proto_version_with_sentinel_ping)
